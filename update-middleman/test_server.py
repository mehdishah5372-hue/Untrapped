...
class FakeResponse:
    def __init__(self, body=b'ok', headers=None):
        self.body=body; self.headers=headers or {}; self.closed=False
    def __enter__(self): return self
    def __exit__(self,*a): self.close()
    def get_content_length(self):
        v=self.headers.get('Content-Length'); return int(v) if v is not None else None
...
class MiddlemanTests(unittest.TestCase):
    def setUp(self):
        server._cache.clear(); server._locks.clear()

    def test_redirect_handler_rejects_redirect(self):
        h=server.NoRedirectHandler()
        req=server.Request('https://example.invalid/a')
        with self.assertRaises(HTTPError) as cm:
            h.redirect_request(req,None,302,'Found',{},'https://evil.invalid/b')
    ...
    def test_deterministic_4xx_not_retried(self):
        err=HTTPError('https://u/x',422,'unprocessable',{},io.BytesIO(b''))
        f=FakeOpener([err]); old=server.OPENER; server.OPENER=f
        try:
    ...
    def test_transient_then_success_retries(self):
        e=HTTPError('https://u/x',503,'busy',{},io.BytesIO(b''))
        f=FakeOpener([e,FakeResponse(b'abc')]); old=server.OPENER; server.OPENER=f
        old_delay=server.time.sleep; server.time.sleep=lambda _:None
        try:
            self.assertEqual(server.fetch('manifest.json'),b'abc'); self.assertEqual(f.calls,2)
        finally: server.OPENER=old; server.time.sleep=old_delay
