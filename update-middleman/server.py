...
def fetch(path: str):
    req = Request(UPSTREAM + path + "?middleman=" + str(time.time_ns()), headers={"User-Agent": "Untrapped-Update-Middleman/1.1"})
...
def build_manifest():
    ...
    for path in ARTIFACTS:
        ...
        entries.append({
            "path": path, "sha256": sha256(native), "bytes": len(native),
            "version": ".".join(map(str, version_of(native))), "normalized": normalized,
            "url": "/v1/artifact/" + path,
        })
    return {
        "service": "Untrapped Update Middleman", "protocol": 1,
        "baseline": "1.0.0", "minimum_baseline": "1.0.0",
        "generated_at": int(time.time()), "artifacts": entries,
    }

class Handler(BaseHTTPRequestHandler):
    server_version = "UntrappedMiddleman/1.1"
    ...
    def do_GET(self):
        try:
            ...
            if clean == "/health":
                self.send_json(200, {"ok": True, "service": "Untrapped Update Middleman", "baseline": "1.0.0", "protocol": 1});return
            ...
            if clean.startswith(prefix):
                ...
                raw = fetch(path);native, normalized = normalize(raw, path)
                ...
                self.send_header("Cache-Control", "no-store");self.send_header("X-Untrapped-SHA256", sha256(native))
                self.send_header("X-Untrapped-Normalized", "true" if normalized else "false")
                self.send_header("X-Untrapped-Baseline", "1.0.0");self.send_header("Content-Length", str(len(native)));self.end_headers();self.wfile.write(native);return
