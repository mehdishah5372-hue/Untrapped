...
try{
 ...
 $mismatchRows=@($rows|Where-Object{$_.classification -eq 'REPORTER_OR_DIAGNOSIS_DIVERGENCE'})
 foreach($m in $mismatchRows){ Stamp ("DIVERGENCE CASE=" + $m.case + " baseline_high=" + $m.high_confidence_baseline + " matched=" + $m.high_confidence_matched); foreach($x in @($m.unmatched_high_confidence)){Stamp ("  BASELINE UNMATCHED: category=" + $x.category + " text=" + $x.text + " fingerprint=" + $x.fingerprint + " exit=" + $x.exit_code + " http=" + $x.http_status + " context=" + $x.context)} }
