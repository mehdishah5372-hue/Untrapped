...
# Non-installing: each implementation runs in an isolated PowerShell process.
...
function Parse([string]$p){$t=$null;$e=$null;[void][System.Management.Automation.Language.Parser]::ParseFile($p,[ref]$t,[ref]$e);@($e)}
function Hash([string]$s){$h=[Security.Cryptography.SHA256]::Create();try{([BitConverter]::ToString($h.ComputeHash([Text.Encoding]::UTF8.GetBytes($s)))).Replace('-','').ToLowerInvariant()}finally{$h.Dispose()}}
function FunctionText([string]$p,[string]$n){$raw=Get-Content -Raw -LiteralPath $p;$t=$null;$e=$null;$a=[System.Management.Automation.Language.Parser]::ParseInput($raw,[ref]$t,[ref]$e);$f=$a.FindAll({param($x)$x -is [System.Management.Automation.Language.FunctionDefinitionAst] -and $x.Name -eq $n},$true)|Select-Object -First 1;if($f){$f.Extent.Text}else{''}}
function Invoke-Isolated([string]$Library,[object]$Case,[string]$Label){
  $runner=Join-Path $env:TEMP ('osb-'+$Label+'-'+[guid]::NewGuid().ToString('N')+'.ps1');$events=Join-Path $env:TEMP ('osb-events-'+[guid]::NewGuid().ToString('N')+'.jsonl');$casefile=Join-Path $env:TEMP ('osb-case-'+[guid]::NewGuid().ToString('N')+'.json')
  $Case|ConvertTo-Json -Depth 12|Set-Content -LiteralPath $casefile -Encoding UTF8
  $lib=$Library.Replace("'","''");$ev=$events.Replace("'","''");$cf=$casefile.Replace("'","''")
  @"
`$ErrorActionPreference='Stop'
. '$lib'
`$ErrorLibraryPath='$ev'
`$c=Get-Content -Raw -LiteralPath '$cf'|ConvertFrom-Json
...
`$records=@(Get-ErrorLibraryRecords|ForEach-Object{[ordered]@{schema=[int]`$_.schema;library_version=[string]`$_.library_version;timestamp_utc='DYNAMIC';source=[string]`$_.source;stream=[string]`$_.stream;artifact=[string]`$_.artifact;category=[string]`$_.category;fingerprint=[string]`$_.fingerprint;text=[string]`$_.text;exit_code=[int]`$_.exit_code;http_status=[int]`$_.http_status;candidate_hash=[string]`$_.candidate_hash;previous_candidate_hash=[string]`$_.previous_candidate_hash;attempt=[int]`$_.attempt;repair_action=[string]`$_.repair_action;syntax_result=[string]`$_.syntax_result;context=[string]`$_.context}})
[pscustomobject]@{selected=@(`$r|ForEach-Object{[string]`$_.fingerprint});records=`$records}|ConvertTo-Json -Depth 20 -Compress
"@|Set-Content -LiteralPath $runner -Encoding UTF8
  try{$out=& powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File $runner 2>&1;if($LASTEXITCODE -ne 0){Fail "$Label failed for $($Case.Name)"};$line=@($out|Where-Object{[string]$_ -match '^\s*\{"selected"'}|Select-Object -Last 1);if(-not $line){Fail "$Label produced no JSON for $($Case.Name)"};$line|ConvertFrom-Json}finally{Remove-Item $runner,$events,$casefile -Force -ErrorAction SilentlyContinue}
}
try{
 Stamp 'OSblocker EXECUTIVE DIAGNOSTIC — EXACT BASELINE VS UPGRADE'
 Stamp "Baseline=$BaselineRef; upgrade=current branch; isolated execution; no installation."
 $base=Join-Path $env:TEMP ('osb-baseline-'+[guid]::NewGuid().ToString('N')+'.ps1');$wc=New-Object Net.WebClient;try{$wc.DownloadString($BaselineUrl)|Set-Content -LiteralPath $base -Encoding UTF8}finally{$wc.Dispose()}
 if((Parse $base).Count -ne 0){Fail 'baseline parse failure'};if((Parse $Current).Count -ne 0){Fail 'upgrade parse failure'}
 $names=@('Get-ErrorFingerprint','Classify-ErrorText','Save-ErrorEvent');$hashes=@();foreach($n in $names){$bh=Hash(FunctionText $base $n);$uh=Hash(FunctionText $Current $n);$hashes+=[ordered]@{function=$n;baseline=$bh;upgrade=$uh;identical=($bh -eq $uh)}}
 $cases=@(
  ...
  [pscustomobject]@{Name='explicit';Lines=@('[ERROR] UARD could not start');Exit=0;Http=0;Genuine=@('[ERROR] UARD could not start')},
  [pscustomobject]@{Name='parser';Lines=@('ParserError: Missing closing }');Exit=0;Http=0;Genuine=@('ParserError: Missing closing }')},
  [pscustomobject]@{Name='access';Lines=@('Access is denied');Exit=0;Http=0;Genuine=@('Access is denied')},
  [pscustomobject]@{Name='not-found';Lines=@('The term foo is not recognized as the name of a cmdlet');Exit=0;Http=0;Genuine=@('The term foo is not recognized as the name of a cmdlet')},
  ...
  [pscustomobject]@{Name='two-errors';Lines=@('Access is denied','ParserError: Missing closing }');Exit=0;Http=0;Genuine=@('Access is denied','ParserError: Missing closing }')},
 ...
 $summary=[ordered]@{status=if($gm -eq 0 -and $uf -le $bf -and (@($hashes|Where-Object{-not $_.identical}).Count -eq 0)){'PASS'}else{'FAIL'};baseline_commit=$BaselineRef;cases=$cases.Count;baseline_events=$be;upgrade_events=$ue;baseline_extra_or_false=$bf;upgrade_extra_or_false=$uf;false_positive_reduction=($bf-$uf);genuine_mismatches=$gm;reporter_core_hashes=$hashes;reporter_contract='schema=1, stream=output-scan, fields unchanged';improvements=@('batch-wide inspection before emission','narrative suppression','concrete error recognition','no aggregate synthetic events');downgrades=@();sustained=@('reporter core','fingerprint algorithm','classification algorithm','schema/stream/field contract','individual genuine error text and metadata');sustained_not_beneficial=@('baseline generic lexical false-positive matching','blanket process/HTTP inheritance for narrative lines','WARNING-as-error classification');report_directory=$Report;timestamp_utc=[DateTime]::UtcNow.ToString('o')}
 $summary|ConvertTo-Json -Depth 40|Set-Content -LiteralPath (Join-Path $Report 'executive-summary.json') -Encoding UTF8
 Stamp "Reporter core identical: $((@($hashes|Where-Object{-not $_.identical}).Count) -eq 0)";Stamp "Baseline extra/false=$bf; upgrade extra/false=$uf; reduction=$($bf-$uf)";Stamp "Genuine baseline diagnosis mismatches=$gm";Stamp "Cases=$($cases.Count); baseline events=$be; upgrade events=$ue"
 if(@($hashes|Where-Object{-not $_.identical}).Count -gt 0){Fail 'reporter core changed'};if($gm -gt 0){Fail 'genuine baseline diagnosis changed'};if($uf -gt $bf){Fail 'upgrade has more extra/false events'}
 Stamp 'EXECUTIVE RESULT: PASS — same reporter core and genuine baseline records; checker is materially more selective.';Stamp ('REPORT: '+$Report);exit 0
