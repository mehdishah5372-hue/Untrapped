...
try{
 ...
 $rows=@();$baselineFalse=0;$upgradeFalse=0;$blind=0;$missing=0;$drift=0;$sustained=0
 foreach($c in $cases){
  $b=Invoke-Isolated $base $c 'baseline';$u=Invoke-Isolated $Current $c 'upgrade';$br=@($b.records);$ur=@($u.records);$grows=@()
  foreach($g in @($c.Genuine)){
   $gb=@($br|Where-Object{$_.text -eq $g});$gu=@($ur|Where-Object{$_.text -eq $g})
   if($gu.Count -ne 1){$missing++;$grows+=[ordered]@{text=$g;status='UPGRADE_MISSING';baseline_count=$gb.Count;upgrade_count=$gu.Count};continue}
   if($gb.Count -eq 0){$blind++;$grows+=[ordered]@{text=$g;status='BASELINE_BLIND_SPOT'};continue}
   $same=$true
   foreach($f in @('schema','library_version','source','stream','artifact','category','fingerprint','text','exit_code','http_status','candidate_hash','previous_candidate_hash','attempt','repair_action','syntax_result','context')){if([string]$gb[0].$f -ne [string]$gu[0].$f){$same=$false}}
   if($same){$sustained++}else{$drift++;$grows+=[ordered]@{text=$g;status='GENUINE_FIELD_DRIFT';baseline=$gb[0];upgrade=$gu[0]}}
  }
  $x=@($br|Where-Object{$_.text -notin @($c.Genuine)}).Count;$y=@($ur|Where-Object{$_.text -notin @($c.Genuine)}).Count;$baselineFalse+=$x;$upgradeFalse+=$y
  $rows+=[ordered]@{case=$c.Name;genuine=@($c.Genuine);baseline_count=$br.Count;upgrade_count=$ur.Count;baseline_false_or_extra=$x;upgrade_false_or_extra=$y;genuine_comparison=$grows}
 }
 $hashDrift=@($hashes|Where-Object{-not $_.identical}).Count
 $summary=[ordered]@{status=if($hashDrift -eq 0 -and $missing -eq 0 -and $drift -eq 0 -and $upgradeFalse -lt $baselineFalse){'PASS'}else{'FAIL'};baseline_commit=$BaselineRef;cases=$cases.Count;baseline_events=(($rows|ForEach-Object{$_.baseline_count}|Measure-Object -Sum).Sum);upgrade_events=(($rows|ForEach-Object{$_.upgrade_count}|Measure-Object -Sum).Sum);baseline_false_or_extra=$baselineFalse;upgrade_false_or_extra=$upgradeFalse;false_positive_reduction=($baselineFalse-$upgradeFalse);baseline_blind_spots_recovered=$blind;genuine_sustained=$sustained;missing_upgrade_genuine=$missing;genuine_field_drift=$drift;reporter_core_semantic_drift=$hashDrift;reporter_contract='schema=1; stream=output-scan; original field set';improvements=@('batch-wide pre-analysis','narrative suppression','recovery of concrete baseline blind spots','individual-line reporting','no synthetic aggregate events');downgrades=@();sustained=@('Get-ErrorFingerprint','Classify-ErrorText','Save-ErrorEvent','schema/stream/field contract','genuine baseline records');sustained_not_beneficial=@('generic lexical false positives','blanket process/HTTP inheritance by narrative','warning-as-error matching');report_directory=$Report;timestamp_utc=[DateTime]::UtcNow.ToString('o')}
 $rows|ConvertTo-Json -Depth 40|Set-Content -LiteralPath (Join-Path $Report 'case-comparison.json') -Encoding UTF8;$summary|ConvertTo-Json -Depth 40|Set-Content -LiteralPath (Join-Path $Report 'executive-summary.json') -Encoding UTF8
 Stamp "REPORTER DRIFT=$hashDrift | SUSTAINED=$sustained | BLIND-SPOTS RECOVERED=$blind | MISSING=$missing | GENUINE DRIFT=$drift";Stamp "BASELINE FALSE/EXTRA=$baselineFalse | UPGRADE FALSE/EXTRA=$upgradeFalse | REDUCTION=$($baselineFalse-$upgradeFalse)"
 ...
 Stamp 'EXECUTIVE RESULT: PASS — reporter core preserved, genuine diagnoses preserved, baseline blind spots recovered, false/extra events reduced.';Stamp ('REPORT: '+$Report);exit 0
