(() => {
  ...
  function decodeComponent(value) {
    if (/%(?![0-9A-Fa-f]{2})/.test(value)) throw new Error("malformed percent encoding");
  ...
  function normaliseConfig(config) {
    const ids = Array.isArray(config?.allowedYouTubeVideoIds)
      ? config.allowedYouTubeVideoIds.map(String)
      : [];
    return {
      allowAdditionalQueryParameters: config?.youtubePolicy?.allowAdditionalQueryParameters !== false,
      allowedIds: new Set(ids.filter((id) => VIDEO_ID_RE.test(id))),
    };
  }
  ...
  function decideYouTubeUrl(input, config) {
    ...
    if (url.port && url.port !== "443") return { ...result, reason: "non-default-port" };
    if (url.username || url.password) return { ...result, reason: "credentials-present" };
    if (url.pathname !== "/watch") return { ...result, reason: "path-not-watch" };
    if (url.hash) return { ...result, reason: "fragment-present" };
  ...
  const api = { decideYouTubeUrl, parseQuery, normaliseConfig };
  if (typeof module !== "undefined" && module.exports) module.exports = api;
  globalThis.UntrappedPolicy = api;
})();
