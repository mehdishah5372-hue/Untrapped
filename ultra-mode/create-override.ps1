...
try{
 ...
 [pscustomobject]@{version='1';payload=$payload;signature=[Convert]::ToBase64String($sig)}|ConvertTo-Json -Compress|Set-Content -Path $OutputPath -Encoding utf8
 Write-Host "Signed override written to: $OutputPath"
}finally{
 if($bstr -ne [IntPtr]::Zero){[Runtime.InteropServices.Marshal]::ZeroFreeBSTR($bstr)}
 Remove-Variable plainPass,pass,keyJson,jsonBytes -ErrorAction SilentlyContinue
 if($aes){$aes.Dispose()};if($rsa){$rsa.Dispose()};if($kdf){$kdf.Dispose()}
}
