...
foreach($sample in $falsePositives){
    if(Legacy-Test $sample){$legacyFalse++}
...
foreach($sample in $truePositives){
    if(Legacy-Test $sample.Text $sample.Exit $sample.Http){$legacyTrue++}
...
Assert ((Get-ErrorLibraryRecords | Where-Object {$_.source -eq 'diagnostic-sandbox'}).Count -gt 0) 'force-first events are persisted in sandbox library'
