...
try{
  $nodeScript='const fs=require(''fs''); const p=require(process.argv[1]); const raw=fs.readFileSync(process.argv[2],''utf8'').replace(/^\uFEFF/,''''); const c=JSON.parse(raw); for(const x of c){let d; try{d=p.decideYouTubeUrl(x.url,{youtubePolicy:{allowAdditionalQueryParameters:true},allowedYouTubeVideoIds:[''AAAAAAAAAAA'']});}catch(e){d={decision:''BLOCK'',reason:''policy-evaluation-error''};} process.stdout.write(JSON.stringify({name:x.name,url:x.url,decision:d.decision,reason:d.reason})+''\n'');}'
