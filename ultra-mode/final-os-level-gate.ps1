...
foreach($required in @('WinDivert.dll','WinDivert64.sys','config.json','packet-filter.ps1','YouTubePolicy.ps1')){if(-not(Test-Path (Join-Path $Root $required))){Fail 'required-artifacts' "$required missing."}}
...
$tokens=$null;$parseErrors=$null;[void][System.Management.Automation.Language.Parser]::ParseInput($packet,[ref]$tokens,[ref]$parseErrors)
...
if($packet -notmatch 'WinDivertOpen' -or $packet -notmatch 'WinDivertClose'){Fail 'windivert-primitives' 'packet-filter.ps1 does not contain both WinDivertOpen and WinDivertClose.'}
if($packet -notmatch 'EmergencyFilter' -or $packet -notmatch 'FAIL-CLOSED'){Fail 'fail-closed-path' 'packet-filter.ps1 lacks the emergency fail-closed path.'}
Pass 'packet-filter-parse-and-safety'

$config=Get-Content (Join-Path $Root 'config.json') -Raw | ConvertFrom-Json
foreach($n in @('enabled','start','end','domains','alwaysBlockedDomains','alwaysAllowedDomains','youtubePolicy','allowedYouTubeVideoIds')){if($null -eq $config.$n){Fail 'json-policy-consumption' "Missing policy property $n."}}
...
if($config.youtubePolicy.allowAdditionalQueryParameters -and @($config.allowedYouTubeVideoIds).Count -gt 0){
  foreach($id in @($config.allowedYouTubeVideoIds)){if([string]$id -cnotmatch '^[A-Za-z0-9_-]{11}$'){Fail 'fail-closed-youtube-policy' "Invalid allowlisted video ID: $id"}}
}
...
$filterText=(Get-Command New-WinDivertFilter -ErrorAction SilentlyContinue)
# Parse the real filter expression generated by packet-filter.ps1's policy function.
$tokens=$null;$errs=$null;$sample=''
...
if($sample -notmatch 'tcp\.DstPort == 443' -or $sample -notmatch 'udp\.DstPort == 443'){Fail 'packet-filter-generation' 'Generated HTTPS/QUIC clauses missing.'}
