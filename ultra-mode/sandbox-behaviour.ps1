...
# This deliberately executes only the policy/helper functions extracted from packet-filter.ps1.
# It does NOT open WinDivert, alter networking, change firewall/WFP/DNS/Hosts/VPN, or install anything.
$ErrorActionPreference='Stop'
$Root=Split-Path -Parent $MyInvocation.MyCommand.Path
$scriptPath=Join-Path $Root 'packet-filter.ps1'
...
if(-not(Test-UltraActive $config ([TimeSpan]::Parse('23:30')))){throw 'Always-block-only schedule model must not be confused with active scheduled window'}
