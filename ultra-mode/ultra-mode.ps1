...
# Uses Windows Defender Firewall Dynamic Keyword Addresses (FQDN rules), hosts,
# and Brave managed policies. Requires an elevated PowerShell session.
...
function Set-BravePolicy($domains){
  ...
  $blockKey=Join-Path $PolicyPath 'URLBlocklist';New-Item $blockKey -Force|Out-Null
  $i=1;foreach($d in $domains){New-ItemProperty $blockKey -Name ([string]$i) -PropertyType String -Value "[*.]$d" -Force|Out-Null;$i++}
...
function Set-FqdnRules($domains){
  if(-not (Get-Command New-NetFirewallDynamicKeywordAddress -ErrorAction SilentlyContinue)){throw 'Windows Firewall Dynamic Keyword support is unavailable on this Windows installation.'}
  Remove-FqdnRules $domains
  foreach($d in $domains){$id='{'+(New-Guid).Guid+'}';New-NetFirewallDynamicKeywordAddress -Id $id -Keyword $d -AutoResolve $true|Out-Null;New-NetFirewallRule -DisplayName ($RulePrefix+$d) -Direction Outbound -Action Block -Profile Any -RemoteDynamicKeywordAddresses $id|Out-Null}
  # Hydrate the FQDN rules through the Windows resolver so the firewall has current addresses.
  foreach($d in $domains){Resolve-DnsName -Name $d -DNSOnly -ErrorAction SilentlyContinue|Out-Null}
}
