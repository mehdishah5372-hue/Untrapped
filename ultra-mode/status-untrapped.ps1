...
function TestTarget443([string]$Name, [bool]$ExpectedBlocked) {
    try {
        ...
        if ($tcp.TcpTestSucceeded) {
            if ($ExpectedBlocked) { Report ('[FAIL REACHABLE] BLOCK TEST: ' + $Name + ' is REACHABLE on TCP 443; expected BLOCKED; RemoteAddress=' + [string]$tcp.RemoteAddress); Problem ($Name + ' is reachable while it should be blocked.') }
            else { Report ('[OK REACHABLE] BLOCK TEST: ' + $Name + ' is reachable on TCP 443; expected ALLOWED; RemoteAddress=' + [string]$tcp.RemoteAddress) }
        } else {
            if ($ExpectedBlocked) { Report ('[OK UNREACHABLE] BLOCK TEST: ' + $Name + ' is BLOCKED on TCP 443 as expected; RemoteAddress=' + [string]$tcp.RemoteAddress) }
            else { Report ('[FAIL UNREACHABLE] BLOCK TEST: ' + $Name + ' failed TCP 443 but should be ALLOWED; RemoteAddress=' + [string]$tcp.RemoteAddress); Problem ($Name + ' is unreachable while it should be allowed.') }
        }
    } catch {
        if ($ExpectedBlocked) { Report ('[OK UNREACHABLE] BLOCK TEST: ' + $Name + ' could not establish TCP 443 as expected for a blocked target.') }
...
Report ''; Report 'BLOCK TESTS'; Report '------------'
$scheduledBlock = ([bool]$config.enabled -and $active -and -not $overrideActive); $alwaysBlocked = $false
