...
function Get-YouTubeRules {
    $path=Join-Path $Root '..\youtube-rules.json'
    if(-not(Test-Path -LiteralPath $path)){throw "Missing youtube-rules.json at $path"}
    try { Get-Content -LiteralPath $path -Raw | ConvertFrom-Json } catch { throw "Invalid youtube-rules.json: $($_.Exception.Message)" }
}
function Test-YouTubeRules {
    $rules=Get-YouTubeRules
    if([int]$rules.version -ne 1){throw 'Unsupported youtube-rules.json version.'}
    if(-not [bool]$rules.policy.allowOnlyListedWatchVideos){throw 'YouTube policy must enforce listed watch videos only.'}
