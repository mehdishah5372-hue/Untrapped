...
try{
 ...
 & powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File (Join-Path $Root 'update-youtube-allowlist.ps1') -InputValue @('dQw4w9WgXcQ','https://www.youtube.com/watch?v=BaW_jenozKc') -PolicyPath $policy; if($LASTEXITCODE -ne 0){throw 'Updater returned non-zero.'}
 ...
 try{& powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File (Join-Path $Root 'update-youtube-allowlist.ps1') -InputValue @('bad-id') -PolicyPath $policy;throw 'Invalid input was accepted.'}catch{if($_.Exception.Message -notmatch 'Invalid YouTube video ID/URL'){throw}}
