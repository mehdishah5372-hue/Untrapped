...
$summary=@()
foreach($n in $files){$p=Join-Path $Root $n;$stage='parse-'+([IO.Path]::GetFileNameWithoutExtension($n));$tokens=$null;$errors=$null;$null=[System.Management.Automation.Language.Parser]::ParseFile($p,[ref]$tokens,[ref]$errors);$ok=@($errors).Count -eq 0;SaveJ $stage 'result.json' ([ordered]@{stage=$stage;file=$n;result=if($ok){'PASS'}else{'FAIL'};errors=@($errors|ForEach-Object{[ordered]@{message=$_.Message;line=$_.Extent.StartLineNumber;column=$_.Extent.StartColumnNumber}})});$summary+=[ordered]@{stage=$stage;result=if($ok){'PASS'}else{'FAIL'};file=$n};if(-not $ok){$errors|ForEach-Object{Write-Error "${n}:$($_.Extent.StartLineNumber):$($_.Extent.StartColumnNumber): $($_.Message)"};exit 1}}
